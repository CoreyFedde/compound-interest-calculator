const calculateInterestOverYear = (principal, rate) => {
  ...
  export const calculateTotalRetirementWithMonthlyContribution = (
    ...
    for (let i = 0; i < investmentYears; i++) {
      ...
      if (i === 0) {
        principal = previousData ? previousData.principal + annualContribution : deposit + annualContribution;
        interest = previousData ? calculateInterestOverYear(previousData.total, rate) : deposit * rate;
        total = previousData ? previousData.total + annualContribution + interest : principal + interest;
      } else {
