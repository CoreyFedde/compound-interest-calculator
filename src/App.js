...
function App() {
  ...
  const [timeRangeValues, setTimeRangeValues] = useState([{start: userValues.age, end: userValues.retirementTarget, period: 1}])
...
const handleTimeRangeChange = e => {
  ...
  const targetRange = timeRangeValues.findIndex(r => r[name] === Number(placeholder));
...
const addTimeRange = e => {
  ...
  const newTimeRangeValues = [...timeRangeValues.length === 1 ? [] : timeRangeValues.slice(0, timeRangeValues.length - 1), {start: previousLastRange.start, end: newEnd, period: previousLastRange.period}, {start: newStart, end: previousLastRange.end, period: previousLastRange.period + 1}]
  return setTimeRangeValues(newTimeRangeValues)
}
const removeTimeRange = e => {
  if (timeRangeValues.length === 2) {
    return setTimeRangeValues([{start: userValues.age, end: userValues.retirementTarget, period: 1}]);
  }
  const previousLastRange = timeRangeValues[timeRangeValues.length - 2];
  const currentRange = previousLastRange.end - previousLastRange.start;
  const newTimeRangeValues = [...timeRangeValues.slice(0, timeRangeValues.length - 2), {start: previousLastRange.start, end: userValues.retirementTarget, period: previousLastRange.period}]
  return setTimeRangeValues(newTimeRangeValues)
}
  ...
  return (
    <div style={{display: 'flex'}}>
      <div style={{flex: 1}}>
        ...
        ____
        {timeRangeValues.map(r => <TimePeriod start={r.start} end={r.end} period={r.period} userValues={userValues} handleChange={handleTimeRangeChange} />)}
