...
const Calculator = (props) => {
    ...
    const calculateCompoundInterestOverYears = (principal, rate = .08, timeInvested = 1, numOfCompound = 1) => {
        const amount = Math.round(principal * (Math.pow((1 + (rate / numOfCompound)), (numOfCompound * timeInvested))));
  ...
  return (
    <div>
      ...
      <div>{calculateTotalRetirementWithMonthlyContribution()[investmentYears - 1].total}</div>
    </div>
  );
}
